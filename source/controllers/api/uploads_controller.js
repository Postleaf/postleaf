  ...
  create: (req, res, next) => {
    ...
    const upload = UploadMiddleware.getMulter({
      ...
      destination: function() {
        ...
        return Path.join(req.app.locals.uploadPath, year, month);
      },
      overwrite: false
    }).single('file');
    ...
    upload(req, res, (err) => {
      ...
      let file = {
        ...
        // Path should be relative to the uploads dir. Example: /2017/01/image.png
        path: req.file.path.substring(req.app.locals.uploadPath.length),
  ...
  download: function(req, res, next) {
    ...
    models.upload
      ...
      .then((upload) => {
        ...
        // Stream the download
        let stream = Fs.createReadStream(Path.join(req.app.locals.uploadPath, upload.path));
        stream.pipe(res);
      })
      .catch((err) => next(err));
  },
  ...
  delete: function(req, res, next) {
    ...
    models.upload
      ...
      .then((upload) => {
        ...
        // Delete the file
        Fs.unlink(Path.join(req.app.locals.uploadPath, upload.path), () => {
